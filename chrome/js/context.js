...
chrome.storage.local.get("paused_playing",function (item){
  if(item['paused_playing']){
    ...
    chrome.storage.local.set({"paused_playing":"playing"});
...
var context=[];
 ...
 context[0].onclickfunc=function(item){//for changing
   ...
   if(paused_playing=="playing"){chrome.storage.local.set({"paused_playing":"paused"},paused_playing_context);}
   if(paused_playing=="paused"){chrome.storage.local.set({"paused_playing":"playing"},paused_playing_context);}
 }
 ...
 context[1].onclickfunc=function(item){//for changing
   ...
   chrome.storage.sync.set({"local_pass":pass.local},function(){
  ...
  context[2].onclickfunc=function(item){//for changing
    ...
    var exim_url = chrome.extension.getURL("/exim/exim.html");
    console.log("Focusing or creating password .html");
...
context.forEach(function(item){
  chrome.contextMenus.create({"type":item.type,"id":item.id,"title":item.title}, function(){console.log("Context menu created successfully");});
});
}
